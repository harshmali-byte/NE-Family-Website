        ...
        <article className="card-surface-light interactive-card interactive-card-light mt-8 p-6 md:p-8">
          ...
          {step === 5 && (
            <>
              ...
              <div className="mt-6 rounded-[var(--radius-card)] border border-[var(--color-border)] bg-slate-50 p-4">
                <p className="card-body card-text-muted-light">
                  Captcha verification: What is {captcha.left} + {captcha.right}?
                </p>
                <input
                  className="mt-3 w-full rounded-[var(--radius-card)] border border-[var(--color-border)] bg-white px-4 py-3 text-sm outline-none transition focus:border-[var(--color-accent)] focus:ring-2 focus:ring-[rgba(34,211,238,0.2)]"
          ...
          {error && (
            <p className="mt-6 rounded-[var(--radius-card)] border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">
              {error}
            </p>
          )}

          {submitted && (
            <p className="mt-6 rounded-[var(--radius-card)] border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-700">
          ...
          <div className="mt-8 flex justify-between">
            <button
              className="rounded-[var(--radius-card)] border border-[var(--color-border)] px-5 py-2 text-sm transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-50"
              disabled={step === 1}
              onClick={prev}
              type="button"
            >
            ...
            {step < 5 ? (
              <button
                className="rounded-[var(--radius-card)] bg-gradient-to-r from-[var(--color-accent)] to-[var(--color-primary)] px-6 py-2 text-sm font-semibold text-white shadow-md transition hover:opacity-90"
                onClick={next}
                type="button"
              >
                Next
              ...
              <button
                className="rounded-[var(--radius-card)] bg-gradient-to-r from-emerald-400 to-emerald-600 px-6 py-2 text-sm font-semibold text-white shadow-md transition hover:opacity-90"
                onClick={submit}
                type="button"
              >
                Submit
...
function FloatingInput({ label, onChange, spacing = 'mt-6', type = 'text', value }) {
  return (
    <div className={`relative ${spacing}`}>
      <input
        className="peer w-full rounded-[var(--radius-card)] border border-[var(--color-border)] bg-transparent px-4 pb-2 pt-5 text-sm outline-none transition focus:border-[var(--color-accent)] focus:ring-2 focus:ring-[rgba(34,211,238,0.2)]"
